            ...
            <?php foreach ($v['options'] as $key => $opt): ?>
                <tr>
                    ...
                    <td>
                        ...
                        <?php
                        // set default checked
                        $check = ($opt['VENDOR_CEK'] ? $opt['VENDOR_CEK'] : '0');
                        $class = (isset($v['class']) ? $v['class'] : ' ' ) . ' ' . str_replace('"', '', json_encode($form->validation[$k]));
                        ?>
                        <label class="radio inline" style="width:auto;">
                            <input type="radio" name="EP_PGD_PENAWARAN_TEKNIS[<?php echo $key; ?>][VENDOR_CEK]" class="<?php echo $class; ?>" value="1" <?php echo $check == '1' ? 'checked' : '' ?> />
                            YA
                        </label>
                        <label class="radio inline" style="width:auto;">
                            <input type="radio" name="EP_PGD_PENAWARAN_TEKNIS[<?php echo $key; ?>][VENDOR_CEK]" class="<?php echo $class; ?>" value="0" <?php echo $check == '0' ? 'checked' : '' ?> />
                            TIDAK
                        </label>
                    </td>
                </tr>
