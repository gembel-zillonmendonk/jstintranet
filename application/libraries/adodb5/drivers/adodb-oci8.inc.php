...
"select constraint_name,r_owner,r_constraint_name 
	...
	where constraint_type = 'R' and table_name = $table $owner";
		...
		foreach($constraints as $constr) {
			$cons = $this->qstr($constr['CONSTRAINT_NAME']);
			$rowner = $this->qstr($constr['R_OWNER']);
			$rcons = $this->qstr($constr['R_CONSTRAINT_NAME']);
			...
			if ($cols && $tabcol) 
				for ($i=0, $max=sizeof($cols); $i < $max; $i++) {
					$arr[$tabcol[$i]['TABLE_NAME']] = $cols[$i]['COLUMN_NAME'].'='.$tabcol[$i]['COLUMN_NAME'];
				}
		}
		$ADODB_FETCH_MODE = $save;
