<?php $this->load->helper('form'); ?>
  ...
  <fieldset class="ui-widget-content">
   <form id="frmHirarki" method="POST" action="hirarki_jabatan/edit/<?php echo $kode_otorisasi; ?>" >
	...
	<input type="number" name="nilai_maks_otorisasi" value="<?php echo $nilai_maks_otorisasi; ?>" id="nilai_maks_otorisasi"   maxlength="22" style=""    />      
	</p>
	<p>
	<?php echo form_label("Mata Uang") ?>
	<select name="mata_uang" type="dropdown" >
		<?php foreach($arr_matauang as $row ) {
		$sel = ($row["MATA_UANG"] == $mata_uang ? " SELECTED " : " ");
		?>
		<option value="<?php echo $row["MATA_UANG"]; ?>" <?php echo $sel; ?> ><?php echo $row["MATA_UANG"]; ?></option>
		<?php
		}
		?>			
	</select>	
